...
"""
python main.py --data hetionet --text_embedding_file pubmedbert_embeddings_768.npy --knowledge_embedding_file poincare_embeddings.npy --global_embedding_file global_embeddings.npy --w_text 0.3 --w_domain 0.5 --w_global 0.2 --num_hidden_layers 2 --iterations 40000 --evaluate_every 1000 --neg_sample_size_eval 100 --model_state_file hetionet_model_state352.pth
"""
...
def main(args):
    ...
    global_embedding_path = (
        f"{args.data}/{args.global_embedding_file}" if args.global_embedding_file is not None else None
    )
    freeze = args.freeze
    ...
    try:
        ...
        print(f"Failed to load Text Embeddings file, random embeddings will be created. Error: {e}")
    ...
    try:
        ...
        print(f"Failed to load Domain Knowledge Embeddings file, random embeddings will be created. Error: {e}")

    # global embeddings (optional)
    try:
        if global_embedding_path is not None:
            global_embeddings = np.load(global_embedding_path)
            print("Loaded Global (community) Embeddings file successfully!")
        else:
            global_embeddings = None
            print("No Global Embeddings file provided; proceeding without it.")
    except Exception as e:
        global_embeddings = None
        print(f"Failed to load Global Embeddings file, proceeding without it. Error: {e}")

    # 3-way weight handling (backward compatibility with existing --w if no input is provided)
    if (args.w_text is None) and (args.w_domain is None) and (args.w_global is None):
        w_text = args.w
        w_domain = 1.0 - args.w
        w_global = 0.0
    else:
        w_text = 0.34 if args.w_text is None else args.w_text
        w_domain = 0.33 if args.w_domain is None else args.w_domain
        w_global = 0.33 if args.w_global is None else args.w_global
        s = w_text + w_domain + w_global
        if s <= 0:
            w_text, w_domain, w_global = 1.0, 0.0, 0.0
        else:
            w_text, w_domain, w_global = w_text / s, w_domain / s, w_global / s
    print(f"fusion weights -> text: {w_text:.3f}, domain: {w_domain:.3f}, global: {w_global:.3f}")
    ...
    model = LinkPredict(num_nodes,
                    ...
                    pretrained_global_embeddings=global_embeddings,
                    freeze=freeze,
                    w_text=w_text,
                    w_domain=w_domain,
                    w_global=w_global)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="global_embedding_file",
        default=None,
        help="Path of global/community embedding for each node",
    )
    ...
    parser.add_argument("--w_text", type=float, default=None,
                        help="Weight for text embeddings in fusion (overrides --w if any of w_* set)")
    parser.add_argument("--w_domain", type=float, default=None,
                        help="Weight for domain embeddings in fusion")
    parser.add_argument("--w_global", type=float, default=None,
                        help="Weight for global/community embeddings in fusion")
